...
export class GobanCanvas extends GobanCore {
    ...
    private __drawSquare(i: number, j: number): void {
        ...
        if (this.scoring_mode && this.score_estimate) {
            ...
            const alpha = Math.abs(est) * 0.8;
            ctx.save();
            ctx.globalAlpha = alpha;
            ctx.lineWidth = Math.ceil(this.square_size * 0.035) - 0.5;
            let r = this.square_size;
            ctx.rect(cx - r / 2, cy - r / 2, r + 1, r + 1);
            ctx.fill();
            ctx.restore();
        }
        ...
        /* Draw stones & hovers */
        {
            if (stone_color  /* if there is really a stone here */
                ...
                if (this.scoring_mode
                    ...
                    transparent = true;
                    stoneAlphaTransparencyValue = 0.1;

                    let min_transparency = 0.2;
                    let max_transparency = 1;


                    if (color === 1) {
                        stoneAlphaTransparencyValue = (this.score_estimate.heat[j][i] + 1) / 2 * (max_transparency - min_transparency) + min_transparency;
                    } else {
                        stoneAlphaTransparencyValue = (-this.score_estimate.heat[j][i] + 1) / 2 * (max_transparency - min_transparency) + min_transparency
                    }
                }
                ...
                if (!(this.autoplaying_puzzle_move && !stone_color)) {
                    ...
                    if (color === 1) {
                        ...
                        ctx.beginPath();
                        ctx.arc(cx, cy, this.theme_stone_radius, 0, 2 * Math.PI + 0.01, true);
                        ctx.strokeStyle = "#888888";
                        ctx.stroke();
                    } else {
